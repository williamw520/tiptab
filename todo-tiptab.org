...
** TODO Filter tab by status (muted, pinned, hidden, normal/private).  Tri-state: none, muted, unmuted.
** TODO Move focus back to Tip Tab after duplicating a tab.
** TODO Clicking on the status buttons of a tab changes the status.
...
<ul>
  ...
  <li>The hotkey (either one) can be used to cycle through the tabs, with the Enter key to select a tab.</li>
