...
** TODO Status icon for container, showing the container icon and name.
** TODO Starting animation is showing full size thumbnail one by one (rather than zoom in each one).
...
** TODO Minimized window bar at the bottom.  Selected windows to limit the ones whose tabs are shown.
** TODO Minimized container bar at the bottom.  Selected containers to limit the ones whose tabs are shown.
