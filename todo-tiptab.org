...
* Feature TODO
** DONE Multi-tab selection for drag and drop to move multiple tabs at the same time.
** DONE Drag and drop multiple tabs across windows.
** DONE Drag and drop multiple tabs across containers.
** DONE Close the first blank tab of new window.
** DONE Bug: When drag and drop to the private container, each tab creates a new private window.  Reuse existing private window.
** DONE Bug: after dragging couple time, dragging the first tab in a window doesn't remove it from the src window.
** DONE Bug: Group container tabs to a new window seems pointing to the last tab of the old window.
** DONE Bug: Drag and drop at-end or in-front of window has messed up UI animation.
** DONE Dynamically capture new window creation under different circumstances.
** DONE Update window title on tab title update.
