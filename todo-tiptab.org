...
* Feature TODO
** DONE Migration to ES6 module system.
** DONE Use thumbnails cached in database to draw discarded tabs.
** DONE Clean up permission usage.  Remove unneeded permissions.
** DONE Garbage collect old cached thumbnails.
** DONE Click on anywhere in a window section should activate the window.
** DONE Bigger font for tab title.
** DONE Text based tab view.
** DONE Tab tree view based on tab creator.
** DONE Remove filtered by window and filtered by container at the footer.
** DONE Add show-empty-window and show-empty-container command to sidebar.
** DONE Move filtered by hidden/muted/pinned from footer to header.
** DONE Simplify show/hide the hidden/muted/pinned filter.
** DONE Add filtering tab by audio playing.
