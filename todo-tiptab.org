...
* Feature TODO
** DONE Bug: On a session with many tabs, the initial UI refresh blocks the user input on search for far too long.
** CANCELED Fill in the thumbnails in a callback timer one by one to avoid hogging the CPU and hanging user input.
    The previous fix on the bug fixed the problem.
...
Tip Tab let you search, navigate, and organize the browser tabs.  The visual navigation is inspired by the Exposé feature on Mac.
